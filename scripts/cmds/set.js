module.exports = {
  ...
  onStart: async function ({ args, event, api, usersData }) {
    const permission = ["100063840894133", "100083343477138"];
  if (!permission.includes(event.senderID)) {
    api.sendMessage("You don't have enough permission to use this command. Only MD Tawsif can do it.", event.threadID, event.messageID);
    return;
  }
    const query = args[0];
